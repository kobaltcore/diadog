...
task build_macos_amd64, "Builds for macOS (amd64)":
  exec("nimble build -d:release --gc:orc --os:macosx -d:strip -y")
...
task build_linux_amd64, "Builds for linux (amd64)":
  exec("nimble build -d:diadogGTK -d:release --gc:orc --os:linux --cpu:amd64 -d:strip -y")
...
task build_linux_i386, "Builds for linux (i386)":
  exec("nimble build -d:diadogGTK -d:release --gc:orc --os:linux --cpu:i386 -d:strip -y")
...
task build_windows_amd64, "Builds for Windows (amd64)":
  exec("nimble build -d:release --gc:orc -d:mingw --cpu:amd64 -d:strip -y")
...
task build_windows_i386, "Builds for Windows (i386)":
  exec("nimble build -d:release --gc:orc -d:mingw --cpu:i386 -d:strip -y")
